	...
	<div class="row clearfix">
		<div class="col-md-12 column">
			...
			<div class="panel panel-default">
				...
				<div class="navbar-header" id="orderid">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown" id="ordertime">
							 <a></a>
...
<script type="text/javascript">
	...
	$(function(){ 	
		$.ajax({
	        ...
	        success:function(result){
	        	console.log(result);
	            build_item_list(result.extend.order);
	        },
	        error:function(){
	        	alert("error!");
	        }
	...
	function build_item_list(re){
    	var td1 = $("<td></td>").append("商品信息");
    	var td2 = $("<td></td>").append("单价");
    	var td3 = $("<td></td>").append("数量");
    	var td4 = $("<td></td>").append("小计");
    	
    	$("<tr></tr>").append(td1).append(td2).append(td3).append(td4).appendTo("#table thead");
    	
    	var re = eval(re);
    	$.each(re,function(index,item){
    		var _img = $("<img></img>").attr("src","${APP_PATH}/"+item.detail.picUrl).attr("width","50px").append(item.detail.itemName);
    		var img = $("<td></td>").append(_img);
    		var price = $("<td></td>").append("￥"+formatCurrency(item.detail.price));
    		var count = $("<td></td>").append("x"+item.count);
        	var _sum = item.detail.price*item.count;	
    		var sum = $("<td></td>").append("￥"+formatCurrency(_sum));
    		
    		
    		$("<tr></tr>").append(img).append(price).append(count).append(sum).appendTo("#table tbody");
    	}); 
    	
    	$("#orderid a").append("订单号："+getParam("OrderId"));
    	$("#user a").append("购买者："+getParam("User"));
    	$("#ordertime a").append("购买时间："+new Date(parseInt(getParam("OrderTime"))).toLocaleString());
    }
